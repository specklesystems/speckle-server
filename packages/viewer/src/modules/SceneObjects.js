...
export default class SceneObjects {
    ...
    async applyFilter(filter) {
        ...
        if (filter === null) {
            ...
            const filteredSolidObjects = await this.applyFilterToGroup(this.allSolidObjects, filter, newGhostedObjects);
            filteredSolidObjects.visible = false;
            newFilteredObjects.add(filteredSolidObjects);

            const filteredLineObjects = await this.applyFilterToGroup(this.allLineObjects, filter, newGhostedObjects);
            newFilteredObjects.add(filteredLineObjects);

            const filteredTransparentObjects = await this.applyFilterToGroup(this.allTransparentObjects, filter, newGhostedObjects);
            newFilteredObjects.add(filteredTransparentObjects);

            const filteredPointObjects = await this.applyFilterToGroup(this.allPointObjects, filter, newGhostedObjects);
            newFilteredObjects.add(filteredPointObjects);

            // group solid objects
            const groupedFilteredSolidObjects = await this.groupSolidObjects(filteredSolidObjects);
    ...
    async groupSolidObjects(threejsGroup) {
        ...
        for (const obj of threejsGroup.children) {
            ...
            for (const mesh of meshes) {
                ...
                let materialId = `${m.type}/${m.vertexColors}/${m.color.toJSON()}/${m.side}/${m.transparent}/${m.opacity}/${m.emissive}/${m.metalness}/${
                    m.roughness
                }/${m.wireframe}`;
        ...
        for (const materialId in materialIdToBufferGeometry) {
            await this.asyncPause();
            const groupGeometry = BufferGeometryUtils.mergeBufferGeometries(materialIdToBufferGeometry[materialId]);
            await this.asyncPause();
