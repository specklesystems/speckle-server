...
export default class Coverter {
  ...
  public async traverse(
    ...
    if (Array.isArray(obj)) {
      for (const element of obj) {
        ...
        if (this.activePromises >= this.maxChildrenPromises) {
          await this.traverse(element, callback, scale, parents, node)
        } else {
          const childPromise = this.traverse(element, callback, scale, parents, node)
      ...
      if (obj.speckle_type.toLowerCase().includes('builtelements')) {
        if (obj['elements']) {
          childrenConversionPromisses.push(
            this.traverse(obj['elements'], callback, scale, obj.__parents, childNode)
          )
          this.activePromises += childrenConversionPromisses.length
          await Promise.all(childrenConversionPromisses)
          this.activePromises -= childrenConversionPromisses.length
    ...
    for (const prop in target) {
      ...
      if (this.activePromises >= this.maxChildrenPromises) {
        await this.traverse(target[prop], callback, scale, obj.__parents, childNode)
      } else {
