...
module.exports = class IFCParser {
  ...
  async populateSpatialNode(node, chunks, closures, depth) {
    depth++
    this.logger.debug(`${this.spatialNodeCount++} nodes generated \r`)
    closures.push([])
  ...
  async getAllProps() {
    ...
    for (let i = 0; i < allLinesCount; i++) {
      this.logger.debug(`${((i / allLinesCount) * 100).toFixed(3)}% props \r`)
  ...
  async createAndSaveMeshes() {
    ...
    this.ifcapi.StreamAllMeshes(this.modelId, async (mesh) => {
      ...
      for (let i = 0; i < placedGeometries.size(); i++) {
        ...
        this.logger.debug(`${(count++).toFixed(3)} geoms generated\r`)
      }
    })
