...
import { reportFunctionRunProgressFactory, reportFunctionRunStatusFactory } from '@/modules/automate/services/runsManagement'
import {
...
export default (FF_AUTOMATE_MODULE_ENABLED
  ? {
    ...
    Mutation: {
      ...
      async automateFunctionRunProgressReport(_parent, { input }, ctx) {
        const { projectId, functionRunId, progress } = input
        const logger = ctx.log.child({
          projectId,
          functionRunId
        })
        ...
        const projectDb = await getProjectDbClient({ projectId: input.projectId })
        const reportFunctionRunProgress = reportFunctionRunProgressFactory({
          getAutomationFunctionRunRecord: getFunctionRunFactory({
            db: projectDb
          }),
          upsertAutomationFunctionRunRecord: upsertAutomationFunctionRunFactory({
          ...
          updateAutomationRun: updateAutomationRunFactory({
            db: projectDb
          }),
          emitEvent: getEventBus().emit
        })
        ...
        return await withOperationLogging(
          async () =>
            await reportFunctionRunProgress({
              projectId,
              runId: functionRunId,
              progress
            }),
          {
            logger,
            operationName: 'automationFunctionRunProgressReport',
            operationDescription: 'Report the incremental progress of a function run'
          }
        )
      },
      automateMutations: () => ({})
