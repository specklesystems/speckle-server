...
export default (app: Router, { executeHooks }: { executeHooks: ExecuteHooks }) => {
  ...
  app.options('/objects/:streamId', corsMiddlewareAllowingAllOrigins())
          ...
          file.on('end', async () => {
            ...
            try {
              ...
              if (!requestDropped)
                res
                  .status(400)
                  .send('Failed to parse data. Batch is not in JSON format.')
              requestDropped = true
            }
      ...
      busboy.on('error', async (err) => {
        ...
        if (!requestDropped)
          res
            .status(400)
            .end(
              '{"error": "Upload request error. The server logs have more details."}'
            )
        requestDropped = true
      })
