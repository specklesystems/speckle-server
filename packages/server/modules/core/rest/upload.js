      ...
      if (mimeType === 'application/gzip') {
        ...
        file.on('end', async () => {
          ...
          if (gzippedBuffer.length > MAX_FILE_SIZE) {
            debug('speckle:error')(
              `[User ${req.context.userId || '-'}] Upload error: Batch size too large (${
            ...
            if (!requestDropped)
              res
                .status(400)
                .send(`File size too large (${gzippedBuffer.length} > ${MAX_FILE_SIZE})`)
            requestDropped = true
          }
          ...
          if (gunzippedBuffer.length > MAX_FILE_SIZE) {
            ...
            if (!requestDropped)
              res
                .status(400)
                .send(`File size too large (${gunzippedBuffer.length} > ${MAX_FILE_SIZE})`)
            requestDropped = true
          }
          ...
          try {
            ...
            debug('speckle:error')(
              `[User ${req.context.userId || '-'}] Upload error: Batch not in JSON format`
            )
          ...
          let promise = createObjectsBatched(req.params.streamId, objs).catch((e) => {
            ...
            if (!requestDropped)
              res
                .status(400)
                .send('Error inserting object in the database. Check server logs for details')
            requestDropped = true
          })
        ...
        file.on('end', async () => {
          ...
          if (buffer.length > MAX_FILE_SIZE) {
            ...
            if (!requestDropped)
              res.status(400).send(`File size too large (${buffer.length} > ${MAX_FILE_SIZE})`)
            requestDropped = true
          }
