...
import { getWorkspaceSsoProviderFactory } from '@/modules/workspaces/repositories/sso'
import { decryptorFactory } from '@/modules/workspaces/helpers/sso'
...
export = FF_WORKSPACES_MODULE_ENABLED
  ? ({
    ...
    WorkspaceSsoProvider: {
      async __resolveType() {
        // TODO: Disambiguate type when we support multiple provider types
        return 'OidcProvider'
      }
    },
    ...
    Workspace: {
      ...
      ssoProvider: async (parent) => {
        const getWorkspaceSsoProvider = getWorkspaceSsoProviderFactory({
          db,
          decrypt: decryptorFactory()
        })
        return await getWorkspaceSsoProvider({ workspaceId: parent.id })
      }
    },
    WorkspaceBilling: {
