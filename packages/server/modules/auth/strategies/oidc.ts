...
import { Issuer, Strategy, type StrategyVerifyCallbackReqUserInfo } from 'openid-client'
import {
  getOidcDiscoveryUrl,
  getOidcClientId,
  getOidcClientSecret,
...
export const oidcStrategyVerifyCallbackFactory = (deps: {
...
}): StrategyVerifyCallbackReqUserInfo<Express.User> => {
  return async (req, tokenSet, userinfo, done) => {
    ...
    const token: Optional<string> = get(req.session, 'inviteId') || req.session.token

    try {
      const email = userinfo['email']
      if (!email) {
        throw new EnvironmentResourceError('No email provided by the OIDC provider.')
      }
...
export const oidcStrategyBuilderFactory =
  (deps: {
    getServerInfo: GetServerInfo
    getUserByEmail: LegacyGetUserByEmail
    findOrCreateUser: FindOrCreateValidatedUser
  ...
  async (
    ...
    passport.use(
      ...
      new Strategy<Express.User>(
        ...
        oidcStrategyVerifyCallbackFactory(deps)
      )
    )
