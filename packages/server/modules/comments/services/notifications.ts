...
async function processNewCommentMentions(newComment: CommentRecord) {
  const mentionedUserIds = collectMentionedUserIds(newComment)
  if (!mentionedUserIds.length) return

  await sendNotificationsForUsers(mentionedUserIds, newComment)
}

async function processUpdatedCommentMentions(
  previousComment: CommentRecord,
  newComment: CommentRecord
) {
  const previouslyMentionedUserIds = collectMentionedUserIds(previousComment)
...
export async function notifyUsersOnCommentEvents() {
  const exitCbs = [
    onCommentEvent(CommentsEvents.Created, async ({ comment }) => {
      await processNewCommentMentions(comment)
    }),
    onCommentEvent(CommentsEvents.Updated, async ({ newComment, previousComment }) => {
      await processUpdatedCommentMentions(previousComment, newComment)
    })
  ]
