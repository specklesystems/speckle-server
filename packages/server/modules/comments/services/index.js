...
const { Streams } = require('@/modules/core/dbSchema')
...
module.exports = {
  ...
  async createComment({ userId, input }) {
    ...
    // Validate access
    if (comment.streamId) {
      const streams = await Streams.knex().select().where(Streams.col.id, comment.streamId)
      if (!streams.length) throw new Error("Specified stream doesn't exist")
    }
