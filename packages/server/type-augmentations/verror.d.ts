/**
 * Mostly the same as @types/verror:1.10, but adjusted so that it doesn't conflict with the
 * new native Error.cause API.
 */
...
declare module 'verror' {
  declare class VError extends Error {
    ...
    /**
     * @deprecated Use `getCause()` from `errorHelper.ts` instead. This prop isn't going to
     * work because it's actually a function and it's only marked as an Error so as to not conflict
     * with the new native Error.cause API.
     */
    cause?: Error
