...
export class MainRingBufferQueue {
  ...
  async enqueue(item: Uint8Array, timeoutMs: number): Promise<boolean> {
    ...
    if (
      ...
      console.warn(
        `Message data (${messageLength} bytes) + prefix (${MainRingBufferQueue.LENGTH_PREFIX_BYTES} bytes) exceeds RingBuffer data capacity (${this.ringBuffer.availableSpaces} bytes). Skipping item.`
      )
      return false
    }
  ...
  async dequeue(timeoutMs: number): Promise<Uint8Array | undefined> {
    ...
    if (messageLength > this.ringBuffer.length) {
      console.error(
        `Dequeue: Declared message length (${messageLength} bytes) exceeds RingBuffer total space (${this.ringBuffer.length}). Possible data corruption.`
      )
      return undefined
    }
