...
export default {
  ...
  methods: {
    isolate() {
      ...
      if (this.isolated) unIsolateObjects2([id], 'ui-res', true)
      else isolateObjects2([id], 'ui-res', true)
    },
    toggleVisibility() {
      const id = this.resource.data.object.id
      if (this.visible) hideObjects2([id], 'ui-res', true)
      else showObjects2([id], 'ui-res', true)
    }
  }
}
</script>
