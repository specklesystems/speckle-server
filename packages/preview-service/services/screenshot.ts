...
export const getScreenshot = async (params: {
  puppeteerClient: PuppeteerClient
  pageToOpenUrl: string
  puppeteerFunctionToEvaluateInBrowser: EvaluateFunc<[unknown[]]>
  objectUrl: string
  boundLogger: Logger
}) => {
  const launchParams = {
  ...
  await params.puppeteerClient.init(launchParams)
  ...
  try {
    ...
    renderOutput = <RenderOutput>(
      await params.puppeteerClient.loadPageAndEvaluateScript(
        params.boundLogger,
        params.pageToOpenUrl,
        params.puppeteerFunctionToEvaluateInBrowser,
        params.objectUrl
      )
    )
  } catch (err) {
  ...
  if (
    ...
    params.boundLogger.error(renderOutput.error, 'Error generating preview.')
    return null
  } else if (
    typeof renderOutput !== 'object' ||
    !('scr' in renderOutput) ||
