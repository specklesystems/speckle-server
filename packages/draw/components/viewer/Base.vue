...
onMounted(async () => {
  ...
  await viewerStore.loadModelByUrl(props.id, props.url)
  if (props.url) {
    const activeSnapshot = canvasStore.getActiveSnapshot(props.id)
    const viewerContainer = canvasStore.getViewerContainer(props.id)
    const viewerConfig = viewerContainer?.snapshotViewerConfigMap[activeSnapshot.id]
    if (viewerContainer && viewerConfig) {
