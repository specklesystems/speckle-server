...
export default defineNuxtRouteMiddleware(
  withParallelization(async (to, from) => {
    ...
    const ssoSessionError = (errors || []).find(
      (e) =>
        e.extensions?.['code'] === WorkspaceSsoErrorCodes.SESSION_MISSING_OR_EXPIRED
    )
    ...
    if (isModelNotFound) {
      return abortNavigation(
        createError({ statusCode: 404, message: 'Model not found' })
      )
    }
