        ...
        <v-row>
          <v-col class="pt-0">
            <v-card class="pa-5" elevation="0" rounded="lg" color="background2">
              ...
              <v-chip-group v-model="selectedBranch" mandatory class="ml-3" active-class="primary--text text--accent-1">
                <v-chip v-for="(branch, i) in branches" :key="i" class="mb-3" small>
                  {{ branch.name }}
                </v-chip>
              </v-chip-group>
              <!-- <v-btn
              ...
              <v-chip-group active-class="primary--text text--accent-1" mandatory>
              ...
              <div class="clear"></div>
              <p v-if="branches[selectedBranch].description" class="subtitle-1 font-weight-light ml-4 mt-2">
                {{ branches[selectedBranch].description }}
              </p>
              <v-btn v-tooltip="'Edit branch details'" small icon style="position: absolute; right: 15px; top: 15px" @click="editBranch">
        ...
        <v-row>
          <v-col>
            <v-card rounded="lg" class="pa-5" elevation="0" color="background2">
              ...
              <v-card-text>
                <p v-if="branches[selectedBranch].commits.items.length === 0" class="subtitle-1 font-weight-light">
                  There are no commits in the
                  {{ branches[selectedBranch].name }} branch just yet, try
                  sending something...
                </p>
                <div v-for="(commit, i) in branches[selectedBranch].commits.items" :key="i">
                  <list-item-commit :commit="commit" :stream-id="stream.id"></list-item-commit>
                  <v-divider v-if="i < branches[selectedBranch].commits.items.length - 1"></v-divider>
                </div>
              </v-card-text>
            </v-card>
          </v-col>
